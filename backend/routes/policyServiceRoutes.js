...
const {uploadPrivateDocument,signedDownloadUrl}=require("../services/privateDocumentStorage");
...
const serviceUpload=multer({storage:multer.memoryStorage(),limits:{fileSize:5*1024*1024}});const serviceDocTypes=new Set(["Address Proof","Bank Proof","Nominee Proof","Appointee Proof","Other Supporting Document"]);const requiredDocs=item=>{if(item.requestType==="Address Change")return ["Address Proof"];if(item.requestType==="Bank Update")return ["Bank Proof"];if(item.requestType==="Nominee Change"){const docs=["Nominee Proof"];const hasMinor=(item.nomineeChange?.nominees||[]).some(n=>{const d=new Date(n.dateOfBirth),t=new Date();let a=t.getFullYear()-d.getFullYear();if(t<new Date(t.getFullYear(),d.getMonth(),d.getDate()))a--;return Number.isFinite(a)&&a<18});if(hasMinor)docs.push("Appointee Proof");return docs}return []};const checklist=item=>requiredDocs(item).map(documentType=>{const versions=(item.supportingDocuments||[]).filter(d=>d.documentType===documentType),latest=versions[versions.length-1];return {documentType,required:true,status:latest?.status||"Required",documentId:latest?._id||null,remarks:latest?.remarks||""}});const notifyService=async(req,item,title,message)=>{const n=await Notification.create({title,message,type:"Policy Service Update",date:new Date().toISOString().split("T")[0],recipientId:item.customerId,actionLabel:"View request",actionUrl:"/policy-services",reference:`service:${item._id}:${Date.now()}`});req.app.get("io")?.to(`user:${String(item.customerId)}`).emit("newNotification",n);};
router.get("/",auth([...STAFF,"customer"]),async(req,res)=>{try{const q=req.user.role==="customer"?{customerId:req.user.id}:{};const items=await PolicyServiceRequest.find(q).sort({createdAt:-1}).lean();res.json(items.map(x=>({...x,documentChecklist:checklist(x)})))}catch(e){res.status(500).json({message:"Service requests load failed"})}});
...
router.post("/:id/documents",auth(["customer"]),serviceUpload.single("file"),async(req,res)=>{try{const item=await PolicyServiceRequest.findOne({_id:req.params.id,customerId:req.user.id}).select("+supportingDocuments.storageKey");if(!item)return res.status(404).json({message:"Service request not found"});if(["Approved","Rejected"].includes(item.status))return res.status(409).json({message:"Reviewed request cannot accept documents"});const documentType=clean(req.body.documentType,60);if(!req.file||!serviceDocTypes.has(documentType)||!["application/pdf","image/jpeg","image/png","image/webp"].includes(req.file.mimetype))return res.status(400).json({message:"Valid supporting document required"});const stored=await uploadPrivateDocument({buffer:req.file.buffer,mimeType:req.file.mimetype,customerId:req.user.id,documentType:"service-"+documentType});item.supportingDocuments.push({documentType,fileName:clean(req.file.originalname,255)||"document",storageKey:stored.public_id,resourceType:stored.resource_type||"raw",status:"Pending"});await item.save();res.status(201).json(item.supportingDocuments[item.supportingDocuments.length-1]);}catch(e){console.error(e);res.status(500).json({message:"Supporting document upload failed"})}});
router.post("/:id/documents/:docId/reupload",auth(["customer"]),serviceUpload.single("file"),async(req,res)=>{try{const item=await PolicyServiceRequest.findOne({_id:req.params.id,customerId:req.user.id}).select("+supportingDocuments.storageKey");if(!item)return res.status(404).json({message:"Service request not found"});if(!["Submitted","Under Review"].includes(item.status))return res.status(409).json({message:"Reviewed request documents are locked"});const old=item.supportingDocuments.id(req.params.docId);if(!old||old.status!=="Rejected")return res.status(409).json({message:"Only a rejected document can be re-uploaded"});if(!req.file||!["application/pdf","image/jpeg","image/png","image/webp"].includes(req.file.mimetype))return res.status(400).json({message:"Valid replacement document required"});const stored=await uploadPrivateDocument({buffer:req.file.buffer,mimeType:req.file.mimetype,customerId:req.user.id,documentType:"service-"+old.documentType});item.supportingDocuments.push({documentType:old.documentType,fileName:clean(req.file.originalname,255)||"document",storageKey:stored.public_id,resourceType:stored.resource_type||"raw",status:"Pending",replacesDocumentId:old._id,reuploadedAt:new Date()});await item.save();await notifyService(req,item,"Supporting document re-uploaded",old.documentType+" was re-uploaded for policy "+item.policyNumber+" and is pending verification.");await writeAudit(req,{action:"SERVICE_DOCUMENT_REUPLOADED",module:"Policy Services",description:old.documentType+" re-uploaded for "+item.policyNumber,targetUserId:item.customerId});res.status(201).json(item.supportingDocuments[item.supportingDocuments.length-1]);}catch(e){console.error(e);res.status(500).json({message:"Document re-upload failed"})}});
router.get("/:id/documents/:docId/file",auth([...STAFF,"customer"]),async(req,res)=>{try{const q={_id:req.params.id};if(req.user.role==="customer")q.customerId=req.user.id;const item=await PolicyServiceRequest.findOne(q).select("+supportingDocuments.storageKey");if(!item)return res.status(404).json({message:"Service request not found"});const d=item.supportingDocuments.id(req.params.docId);if(!d||!d.storageKey)return res.status(404).json({message:"Document not found"});const upstream=await fetch(signedDownloadUrl(d.storageKey));if(!upstream.ok)return res.status(502).json({message:"Secure document unavailable"});const body=Buffer.from(await upstream.arrayBuffer());res.setHeader("Content-Type",upstream.headers.get("content-type")||"application/octet-stream");res.setHeader("X-Content-Type-Options","nosniff");res.setHeader("Cache-Control","private, no-store");return res.send(body);}catch(e){res.status(500).json({message:"Document access failed"})}});
router.patch("/:id/documents/:docId/review",auth(STAFF),async(req,res)=>{try{const item=await PolicyServiceRequest.findById(req.params.id).select("+supportingDocuments.storageKey");if(!item)return res.status(404).json({message:"Service request not found"});const d=item.supportingDocuments.id(req.params.docId);if(!d)return res.status(404).json({message:"Document not found"});const status=clean(req.body.status,20),remarks=clean(req.body.remarks);if(!["Verified","Rejected"].includes(status)||status==="Rejected"&&!remarks)return res.status(400).json({message:"Valid review required"});d.status=status;d.remarks=remarks;d.reviewedBy=req.user.id;d.reviewedAt=new Date();await item.save();await notifyService(req,item,"Supporting document "+status.toLowerCase(),d.documentType+" for policy "+item.policyNumber+" was "+status.toLowerCase()+".");res.json(d);}catch(e){res.status(500).json({message:"Document review failed"})}});
router.patch("/:id/review",auth(STAFF),async(req,res)=>{try{const item=await PolicyServiceRequest.findById(req.params.id);if(!item)return res.status(404).json({message:"Request not found"});const status=clean(req.body.status,20),remarks=clean(req.body.adminRemarks);if(!["Under Review","Approved","Rejected"].includes(status))return res.status(400).json({message:"Invalid review status"});if(status==="Rejected"&&!remarks)return res.status(400).json({message:"Rejection reason is required"});if(["Approved","Rejected"].includes(item.status))return res.status(409).json({message:"This service request is already closed"}); if(status==="Approved"&&["Address Change","Contact Update","Bank Update"].includes(item.requestType)){const required=requiredDocs(item),missing=required.filter(t=>{const versions=(item.supportingDocuments||[]).filter(d=>d.documentType===t);const latest=versions[versions.length-1];return !latest||latest.status!=="Verified"});if(missing.length)return res.status(400).json({message:"Required documents pending: "+missing.join(", ")});
 ...
 if(status==="Approved"&&item.requestType==="Nominee Change"){const required=requiredDocs(item),missing=required.filter(t=>{const versions=(item.supportingDocuments||[]).filter(d=>d.documentType===t);const latest=versions[versions.length-1];return !latest||latest.status!=="Verified"});if(missing.length)return res.status(400).json({message:"Required documents pending: "+missing.join(", ")});
