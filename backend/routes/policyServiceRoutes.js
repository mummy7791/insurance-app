...
const serviceUpload=multer({storage:multer.memoryStorage(),limits:{fileSize:5*1024*1024}});const serviceDocTypes=new Set(["Address Proof","Bank Proof","Nominee Proof","Other Supporting Document"]);const notifyService=async(req,item,title,message)=>{const n=await Notification.create({title,message,type:"Policy Service Update",date:new Date().toISOString().split("T")[0],recipientId:item.customerId,actionLabel:"View request",actionUrl:"/policy-services",reference:`service:${item._id}:${Date.now()}`});req.app.get("io")?.to(`user:${String(item.customerId)}`).emit("newNotification",n);};
...
router.patch("/:id/review",auth(STAFF),async(req,res)=>{try{const item=await PolicyServiceRequest.findById(req.params.id);if(!item)return res.status(404).json({message:"Request not found"});const status=clean(req.body.status,20),remarks=clean(req.body.adminRemarks);if(!["Under Review","Approved","Rejected"].includes(status))return res.status(400).json({message:"Invalid review status"});if(status==="Rejected"&&!remarks)return res.status(400).json({message:"Rejection reason is required"});if(["Approved","Rejected"].includes(item.status))return res.status(409).json({message:"This service request is already closed"}); if(status==="Approved"&&["Address Change","Contact Update","Bank Update"].includes(item.requestType)){const docs=item.supportingDocuments||[];if(!docs.length||docs.some(d=>d.status!=="Verified"))return res.status(400).json({message:"All supporting documents must be uploaded and verified before approval"});
 ...
 if(status==="Approved"&&item.requestType==="Nominee Change"){const docs=item.supportingDocuments||[];if(!docs.length||docs.some(d=>d.status!=="Verified"))return res.status(400).json({message:"All supporting documents must be uploaded and verified before approval"});
