// frontend/src/admin/AssignPolicy.tsx
...
export default function AssignPolicy() {
  ...
  const convertPhotoToBase64 = (file: File) => {
  ...
  const assignPolicy = async () => {
    if (!form.name.trim() || !form.email.trim() || !form.policyNo.trim()) {
      alert("Name, Email, Policy No required");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/customer/assign-policy`, {
        method: "POST",
        headers: {
      ...
      <div className="form-grid">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (file) convertPhotoToBase64(file);
          }}
        />

        {form.photo && (
          <div className="photo-preview-box">
            <img src={form.photo} alt="Customer" />
          </div>
        )}
        ...
        <input
          placeholder="Advisor / Unit Manager"
          value={form.advisor}
        ...
        <input
          ...
          value={form.agencyManager}
          onChange={(e) => updateForm("agencyManager", e.target.value)}
        />
        ...
        <input
          ...
          value={form.policyNo}
          onChange={(e) => updateForm("policyNo", e.target.value)}
        />

        <select
          value={form.policyType}
          onChange={(e) => updateForm("policyType", e.target.value)}
        >
        ...
        <input
          placeholder="Premium ₹12,500 / Year"
          value={form.premium}
          onChange={(e) => updateForm("premium", e.target.value)}
        />

        <input
          placeholder="Coverage ₹10,00,000"
          value={form.coverage}
        ...
        <input
          placeholder="Members Self, Spouse, Mother"
          value={form.members}
          onChange={(e) => updateForm("members", e.target.value)}
        />

        <input
          placeholder="Last Payment"
          value={form.lastPayment}
          onChange={(e) => updateForm("lastPayment", e.target.value)}
        />

        <input
          placeholder="Next Premium"
          value={form.nextPremium}
          onChange={(e) => updateForm("nextPremium", e.target.value)}
        />
