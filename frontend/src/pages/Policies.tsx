...
{!isCustomer && (
      ...
      {isCustomer&&journeyPlan&&<div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",zIndex:2200,display:"flex",alignItems:"center",justifyContent:"center",padding:18}} onClick={()=>setJourneyPlan(null)}><div className="section" style={{width:"min(900px,100%)",maxHeight:"90vh",overflowY:"auto",margin:0}} onClick={e=>e.stopPropagation()}><div className="section-heading-row"><div><span className="eyebrow">POLICY JOURNEY</span><h2>{journeyPlan.planName}</h2><p className="section-copy">{journeyPlan.policyNumber}</p></div><button className="mini-btn" onClick={()=>setJourneyPlan(null)}>Close</button></div>
