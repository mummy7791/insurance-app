...
setForm(p=>({...p,...saved,customerName:saved.customerName||profileRes.data?.name||"",customerEmail:profileRes.data?.email||"",customerPhone:saved.customerPhone||profileRes.data?.phone||"",address:saved.address||profileRes.data?.address||""}));}catch(e){alert(errorMessage(e,"Unable to open this insurance plan"));navigate("/insurance-plans",{replace:true});}finally{setLoading(false);}},[navigate,planId,quotationId]);
 ...
 useEffect(()=>{if(!plan)return;if(plan.productGroup==="Traditional Products"&&Number(traditionalPremium)>=30000&&!form.dateOfBirth){const annual=Number(traditionalPremium);const instalment=premiumFrequency==="Monthly"?Math.round(annual/12):premiumFrequency==="Quarterly"?Math.round(annual/4):premiumFrequency==="Half-Yearly"?Math.round(annual/2):annual;setQuote({age:0,frequency:premiumFrequency,annualPremium:annual,instalmentPremium:instalment,coverageAmount:Math.round(annual*10.39),deathBenefit:Math.round(annual*10.39),maturityAmount:Math.round(annual*10),guaranteedIncome:Math.round(annual*0.1242),immediateIncome:Math.round(annual*0.30)});return;}if(!form.dateOfBirth)return;const timer=window.setTimeout(()=>{void refreshQuote(form.dateOfBirth,premiumFrequency);},plan.productGroup==="Traditional Products"?350:0);return()=>window.clearTimeout(timer);},[plan,form.dateOfBirth,premiumFrequency,traditionalPremium,refreshQuote]);
 ...
 return <MainLayout title="Insurance Proposal" subtitle="Complete KYC, nominee and settlement details before payment">
  ...
  <div className="proposal-layout"><div>
   <div className="section"><span className="eyebrow">PERSONAL DETAILS</span><h2>Policyholder details</h2><div className="form-grid">
    ...
    {isTraditional?<label><span style={{display:"block",fontSize:13,fontWeight:700,marginBottom:6}}>Annual Premium Amount</span><input type="number" min={30000} step={1000} value={traditionalPremium} onChange={e=>setTraditionalPremium(e.target.value)} placeholder="Minimum ₹30,000"/></label>:<div><span style={{display:"block",fontSize:13,fontWeight:700,marginBottom:6}}>Premium Amount</span><strong>{quoteLoading?"Calculating...":quote?`₹${quote.instalmentPremium.toLocaleString("en-IN")} / ${quote.frequency}`:"Select DOB to calculate"}</strong></div>}
