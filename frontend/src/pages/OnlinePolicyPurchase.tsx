...
type Plan = { _id:string; planName:string; category:string; yearlyAmount?:number; yearlyPremium?:number; coverageAmount?:number; paymentYears?:number; benefits?:string|string[]; coverage?:string; description?:string; premiumFrequencies?:string[]; ageMin?:number; ageMax?:number; };
type BuyForm = {
...
export default function OnlinePolicyPurchase(){
 ...
 const [quote,setQuote]=useState<{age:number;frequency:string;annualPremium:number;instalmentPremium:number}|null>(null); const [quoteLoading,setQuoteLoading]=useState(false);
...
setForm(p=>({...p,...saved,customerName:saved.customerName||profileRes.data?.name||"",customerEmail:profileRes.data?.email||"",customerPhone:saved.customerPhone||profileRes.data?.phone||"",address:saved.address||profileRes.data?.address||""}));}catch(e){alert(errorMessage(e,"Unable to open this insurance plan"));navigate("/insurance-plans",{replace:true});}finally{setLoading(false);}},[navigate,planId,quotationId]);
 ...
 const getAge=(dob:string)=>{if(!dob)return 0;const d=new Date(dob);if(Number.isNaN(d.getTime()))return 0;const now=new Date();let age=now.getFullYear()-d.getFullYear();const m=now.getMonth()-d.getMonth();if(m<0||(m===0&&now.getDate()<d.getDate()))age--;return age;};
 const refreshQuote=useCallback(async(dob:string,frequency="Yearly")=>{const age=getAge(dob);if(!planId||age<=0){setQuote(null);return;}try{setQuoteLoading(true);const r=await api.post<{age:number;frequency:string;annualPremium:number;instalmentPremium:number}>(`/insurance-plans/${planId}/quote`,{age,frequency,coverageAmount:plan?.coverageAmount});setQuote(r.data);}catch(e){setQuote(null);alert(errorMessage(e,"Premium could not be calculated for this age"));}finally{setQuoteLoading(false);}},[planId,plan?.coverageAmount]);
 useEffect(()=>{if(plan&&form.dateOfBirth)void refreshQuote(form.dateOfBirth,quote?.frequency||"Yearly");},[plan,form.dateOfBirth]);
 ...
 return <MainLayout title="Insurance Proposal" subtitle="Complete KYC, nominee and settlement details before payment">
  ...
  <div className="proposal-layout"><div>
   <div className="section"><span className="eyebrow">PERSONAL DETAILS</span><h2>Policyholder details</h2><div className="form-grid">
    ...
    {form.dateOfBirth&&<div className="proposal-full"><strong>Age: {getAge(form.dateOfBirth)}</strong> · <span>{quoteLoading?"Calculating premium...":quote?`Annual premium ₹${quote.annualPremium.toLocaleString("en-IN")}`:"Enter an eligible date of birth"}</span></div>}<input className="proposal-full" placeholder="Residential Address" value={form.address} onChange={e=>update("address",e.target.value)}/><input className="proposal-full" placeholder="Advisor Code (Optional)" value={form.advisorCode} onChange={e=>update("advisorCode",e.target.value.toUpperCase())}/></div><p className="kyc-note">Advisor code is optional. If entered, this policy business will be linked to that active advisor after successful payment.</p></div>
