...
export default function UserManagement() {
  ...
  const downloadBankPdf=async(item:BankSubmission)=>{try{const r=await api.get(`/advisor-bank/${item._id}/pdf`,{responseType:"blob"});const url=URL.createObjectURL(new Blob([r.data],{type:"application/pdf"}));const a=document.createElement("a");a.href=url;a.download=`advisor-bank-${item.advisorCode||item._id}.pdf`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch{alert("Unable to download bank details PDF")}};
