...
export default function PremiumCalculator(){
 const [plans,setPlans]=useState<Plan[]>([]),[planId,setPlanId]=useState(""),[age,setAge]=useState("30"),[dob,setDob]=useState(""),[customerName,setCustomerName]=useState(""),[mobile,setMobile]=useState(""),[gender,setGender]=useState("Male"),[smoker,setSmoker]=useState(false),[cover,setCover]=useState(""),[frequency,setFrequency]=useState("Yearly"),[quote,setQuote]=useState<Quote|null>(null),[loading,setLoading]=useState(false);
 ...
 const downloadQuote=()=>{if(!quote)return;const doc=new jsPDF();const moneyPdf=(n:number)=>"Rs. "+Number(n||0).toLocaleString("en-IN");let y=20;
  doc.setFontSize(18);doc.text("SecureLife Insurance - Smart Quote",20,y);y+=10;doc.setFontSize(10);doc.text(`Quotation No: ${quotationNumber}`,20,y);doc.text(`Date: ${quoteDate.toLocaleDateString("en-IN")}`,120,y);y+=6;doc.text(`Valid Until: ${validityDate.toLocaleDateString("en-IN")}`,20,y);y+=8;doc.setFontSize(11);if(customerName){doc.text(`Customer: ${customerName}`,20,y);y+=7;}if(mobile){doc.text(`Mobile: ${mobile}`,20,y);y+=7;}if(dob){doc.text(`DOB: ${new Date(dob).toLocaleDateString("en-IN")}`,20,y);y+=7;}doc.text(`Plan: ${quote.planName}`,20,y);y+=7;doc.text(`Product: ${quote.productGroup}`,20,y);y+=7;doc.text(`Age: ${quote.age} | Gender: ${quote.gender} | Smoker: ${quote.smoker?"Yes":"No"}`,20,y);y+=7;doc.text(`Benefit / Sum Assured: ${moneyPdf(quote.coverageAmount)}`,20,y);y+=7;doc.text(`Policy Term: ${quote.policyTermYears} years | Premium Paying Term: ${quote.paymentYears} years`,20,y);y+=7;doc.text(`Payment Mode: ${quote.frequency}`,20,y);y+=10;
 ...
 return <div className="calculator-page"><div className="calculator-shell">
  ...
  {quote&&<div className="section"><span className="eyebrow">YOUR QUOTATION</span><h2>{quote.planName}</h2><p className="muted-copy">{quotationNumber} · {quoteDate.toLocaleDateString("en-IN")} · Valid until {validityDate.toLocaleDateString("en-IN")}{customerName ? ` · ${customerName}` : ""}</p><div className="cards">
