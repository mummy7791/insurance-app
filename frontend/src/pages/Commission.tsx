...
export default function Commission(){
 ...
 return <MainLayout title={isAdvisor?"Advisor Business & Earnings":"Advisor Commission Control"} subtitle={isAdvisor?"Policies linked to your advisor code, earnings and payout status":"Set plan-wise rates and approve advisor payout requests"}>
  <div className="cards admin-kpi-grid"><div className="card"><h3>Total Premium Business</h3><h1>{money(business)}</h1></div><div className="card"><h3>Active Policies</h3><h1>{filtered.length}</h1></div><div className="card"><h3>Earned Commission</h3><h1>{money(total)}</h1></div><div className="card"><h3>Paid Commission</h3><h1>{money(paid)}</h1></div><div className="card"><h3>Pending Commission</h3><h1>{money(pending)}</h1></div></div>
  ...
  <div className="section admin-table-section"><div className="section-heading-row"><div><span className="eyebrow">SETTLEMENT REGISTER</span><h2>{isAdvisor?"My earnings & settlement history":"Advisor payout requests & history"}</h2></div><button className="mini-btn" onClick={()=>void load()}>Refresh</button></div>
   ...
   {loading?<p>Loading...</p>:filtered.length===0?<div className="admin-empty-state"><strong>No advisor business found</strong><span>Paid policies linked through an advisor code will appear here.</span></div>:<div className="premium-table-wrap advisor-commission-table"><table className="table"><thead><tr>{!isAdvisor&&<th>Advisor</th>}<th>Customer / Plan</th><th>Policy</th><th>Premium Business</th><th>Rate</th><th>Commission</th><th>Purchase / Next Due</th><th>Status</th><th>Action / Remarks</th></tr></thead><tbody>{filtered.map(x=><tr key={x._id}>
    {!isAdvisor&&<td><strong>{x.employeeName}</strong><br/><small>{x.advisorCode||"—"}</small></td>}<td>{x.customerName}<br/><small>{x.planName||"Plan"}</small></td><td>{x.policyNumber}<br/><small>{x.policyStatus||"Active"}</small></td><td><strong>{money(x.premiumAmount)}</strong></td><td>{Number(x.commissionRate||0).toLocaleString("en-IN",{maximumFractionDigits:2})}%</td><td><strong>{money(x.commissionAmount)}</strong></td><td>{date(x.purchaseDate)}<br/><small>Due: {date(x.nextPremiumDate)}</small></td><td><span className="badge">{x.status}</span>{x.paidDate&&<><br/><small>Paid: {date(x.paidDate)}</small></>}</td><td>{isAdvisor&&["Eligible","Rejected"].includes(x.status)&&<button className="mini-btn" onClick={()=>void requestPayout(x)}>Request Payout</button>}{isAdmin&&x.status==="Requested"&&<><button className="mini-btn" onClick={()=>void review(x,"Paid")}>Approve & Paid</button> <button className="mini-btn" onClick={()=>void review(x,"Rejected")}>Reject</button></>}<br/><small>{x.remarks||"—"}</small></td>
   </tr>)}</tbody></table></div>}
  </div>
 </MainLayout>;
}
